...
const ProductModal = ({ isOpen, product, setIsModalOpen, setSelected, cart_id, setCart }) => {
    ...
    function addToCart(){
        ...
        axiosClient.post(`/addtocart/${cart_id}`, data)
            .then( (res) => {
                console.log(res);
                setCart((prev) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 px-4">
            <div className="bg-white p-4 rounded-lg shadow-lg max-w-md w-full max-h-[90vh] overflow-y-scroll">
                ...
                <img
                    src={`${ipconfig}/storage/${product.image}`}
                    alt={product.name}
...
function UserHome() {
    ...
    const { userData, setUserData, cart, setCart } = useOutletContext();
    console.log(cart);
            ...
            <div key={item.id} className='p-2 rounded w-1/2 md:w-1/3 lg:w-1/4 min-h-[300px]'>
                <div className='min-h-full flex flex-col justify-between rounded-lg shadow-md' onClick={() => handleProductClick(item)}>    
                    <div
                        ...
                        style={{
                            backgroundImage: `url(${ipconfig}/storage/${item.image})`,
                        }}
                    >
                    </div>
    ...
    return (
        <div className="pt-14 p-2 min-h-screen mb-20 lg:px-40 flex flex-col items-center">
            <div className='mt-3'>
